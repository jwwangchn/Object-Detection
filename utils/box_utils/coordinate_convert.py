...
def forward_convert(coordinate, with_label=False):
    """
    :param coordinate: format [cx, cy, w, h, theta, (label)]
    :param 
    :return: format [x1, y1, x2, y2, x3, y3, x4, y4 (label)]
    """
    boxes = []
    if with_label:
...
def back_forward_convert(coordinate, with_label=True):
    ...
    :return: format [cx, cy, w, h, theta, (label)]
    """
...
if __name__ == '__main__':
    ...
    coord1 = np.array([[10, 10, -10, 10, -10, -10, 10, -10, 1]])

    coord2 = forward_convert(coord, with_label=True)
    coord3 = back_forward_convert(coord1, with_label=True)
